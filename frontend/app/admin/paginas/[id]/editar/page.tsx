...
function AdminEditPageContent({ pageId }: { pageId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateMutation.mutateAsync({ id: pageId, ...formData })
  ...
  const handleSaveDraft = async () => {
    try {
      await updateMutation.mutateAsync({ id: pageId, ...formData, published: false })
      router.push('/admin/paginas')
    } catch (error) {
  ...
  const handlePublish = async () => {
    try {
      await updateMutation.mutateAsync({ id: pageId, ...formData, published: true })
      router.push('/admin/paginas')
    } catch (error) {
  ...
  if (!page) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          <p className="text-gray-400 text-lg">Página não encontrada</p>
          <Link href="/admin/paginas" className="btn-primary mt-4 inline-block">
            Voltar para Páginas
          </Link>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-400 mt-2">Edite as informações da página</p>
          </div>
        </div>
...
export default function AdminEditPagePage({ params }: { params: { id: string } }) {
  return (
    <ProtectedRoute requireAdmin>
      <AdminEditPageContent pageId={params.id} />
    </ProtectedRoute>
  )
}
